...
// 자동 템플릿 생성 함수
async function createSportTemplates(sportCode, templateName) {
  try {
    ...
    const sourceTemplates = [
      'soccer-template.ejs',
      'soccer-control.ejs', 
      'soccer-control-mobile.ejs'
    ];
    
    const targetTemplates = [
      sportCode + '-template.ejs',
      sportCode + '-control.ejs',
      sportCode + '-control-mobile.ejs'
    ];
    
    // views 폴더에 템플릿 파일들 복사
    for (let i = 0; i < sourceTemplates.length; i++) {
      const sourcePath = path.join(viewsDir, sourceTemplates[i]);
      const targetPath = path.join(viewsDir, targetTemplates[i]);
      
      if (fsSync.existsSync(sourcePath)) {
        await fs.copyFile(sourcePath, targetPath);
        logger.info('템플릿 파일 복사: ' + targetTemplates[i]);
      } else {
        logger.warn('소스 템플릿 파일이 없습니다: ' + sourceTemplates[i]);
      }
    }
    
    // public 폴더에 필요한 디렉토리들 생성
    const directories = [
      path.join(publicDir, 'TEAMLOGO', sportCode),
      path.join(publicDir, 'overlay-images', sportCode)
    ];
    ...
    // team_logo_map.json 파일 생성
    const teamLogoMapPath = path.join(publicDir, 'TEAMLOGO', sportCode, 'team_logo_map.json');
    const initialLogoMap = {
      teamLogoMap: {}
    };
    ...
    logger.error('템플릿 생성 실패: ' + error.message);
    return false;
  }
}
...
app.post('/api/sport', async (req, res) => {
  try {
    ...
    // 자동으로 템플릿 파일들 생성
    const templateCreated = await createSportTemplates(sportCode, name);
    
    if (templateCreated) {
      logger.info(`새 종목 생성 및 템플릿 자동 생성 완료: ${name} (${sportCode})`);
    } else {
      logger.warn(`새 종목 생성 완료했으나 템플릿 생성 실패: ${name} (${sportCode})`);
    }
